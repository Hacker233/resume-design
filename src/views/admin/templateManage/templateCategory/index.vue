...
<script lang="ts" setup>
  ...
  import { getTemplateCategoryListAsync, templateCategoryDeleteAsync } from '@/http/api/resume';
  ...
  const getTemplateCategoryList = async () => {
    const data = await getTemplateCategoryListAsync();
  ...
  const deleteTemplate = (row: any) => {
    ElMessageBox.confirm('删除该分类后无法恢复，确定继续？', '警告', {
      ...
      .then(async () => {
        const data = await templateCategoryDeleteAsync(row._id);
        if (data.data.status === 200) {
