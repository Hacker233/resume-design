  ...
  >
    ...
    >
      <el-form-item label="审核状态">
        <el-select v-model="ruleForm.audit_status" placeholder="请选择审核状态" size="default">
          <el-option
...
<script lang="ts" setup>
  import { auditTemplateByAdminAsync } from '@/http/api/lego';
  ...
  watch(
    ...
    (newVal) => {
      if (newVal) {
        console.log(newVal);
        ruleForm.audit_status = newVal.audit_status;
      }
    },
    {
      deep: true
  ...
  const submit = async (formEl: FormInstance | undefined) => {
    ...
    await formEl.validate(async (valid, fields) => {
      if (valid) {
        let params = {
          audit_status: ruleForm.audit_status,
          id: props.auditRow._id
        };
        sureLoading.value = true;
        const data = await auditTemplateByAdminAsync(params);
