...
<script lang="ts" setup>
  import { addArticleCategoryAsync, articleCategoryUpdateAsync } from '@/http/api/article';
    ...
    await formEl.validate(async (valid, fields) => {
      if (valid) {
        ...
        if (props.btnText === '添加') {
          ...
          const data = await addArticleCategoryAsync(params);
          ...
          const data = await articleCategoryUpdateAsync(params);
          if (data.data.status === 200) {
