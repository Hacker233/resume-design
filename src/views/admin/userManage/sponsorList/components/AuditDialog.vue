  ...
  >
    ...
    >
      <el-form-item label="审核状态">
        <el-select v-model="ruleForm.vaild" placeholder="请选择审核状态" size="default">
          <el-option
...
<script lang="ts" setup>
  import { auditSponsorAsync } from '@/http/api/sponsor';
  ...
  const submit = async (formEl: FormInstance | undefined) => {
    ...
    await formEl.validate(async (valid, fields) => {
      if (valid) {
        let params = {
          vaild: ruleForm.vaild,
          id: props.row.id
        };
        sureLoading.value = true;
        const data = await auditSponsorAsync(params);
