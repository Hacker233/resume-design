...
<script lang="ts" setup>
  import { addAdminMenuAsync, updateAdminMenuAsync } from '@/http/api/menu';
    ...
    await formEl.validate(async (valid, fields) => {
      if (valid) {
        ...
        if (props.btnText === '新增') {
          ...
          const data = await addAdminMenuAsync(params);
          ...
          const data = await updateAdminMenuAsync(params);
          if (data.data.status === 200) {
  ...
  const handleChangeIconCheckbox = async (value: boolean) => {
    if (value) {
      ruleForm.iconfont = '';
    } else {
