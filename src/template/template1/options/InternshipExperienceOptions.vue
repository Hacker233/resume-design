      ...
      <el-form label-width="70px" label-position="left">
        ...
        <div v-for="(item, index) in data.LIST" :key="index" class="campus-list">
          ...
          <el-form-item label="公司名称:">
            ...
            <el-switch v-model="data.isShow.companyName" />
          </el-form-item>
...
<script setup lang="ts">
  ...
  const data = reactive<IINTERNSHIPEXPERIENCE>(resumeJsonNewStore.COMPONENTS[index].data); // 组件数据
  ...
  const addContent = (index: number): void => {
    data.LIST[index].jobContent.push({
      content: '主要工作内容是打杂'
    });
  };
  // 删除内容
  const deleteContent = (index: number, cIndex: number): void => {
    data.LIST[index].jobContent.splice(cIndex, 1);
  };
</script>
