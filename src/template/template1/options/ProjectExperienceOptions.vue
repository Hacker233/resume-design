...
import { IMATERIALITEM } from '@/interface/material';
  defineOptions({ name: 'PROJECT_EXPERIENCE' });
  // store
  ...
  const data = reactive<IPROJECTEXPERIENCE>(resumeJsonNewStore.COMPONENTS[index].data); // 组件数据
  ...
  const addContent = (index: number): void => {
    data.LIST[index].projectContent.push({
      content: '主要工作内容是打杂'
    });
  };
  // 删除内容
  const deleteContent = (index: number, cIndex: number): void => {
    data.LIST[index].projectContent.splice(cIndex, 1);
  };
</script>
